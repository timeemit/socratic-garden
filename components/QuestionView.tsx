...
export default class QuestionView extends React.Component<Props, State> {
  ...
  onChoice = (choice: ChoiceIndex) => {
    ...
    let correct_choice_depth = JSON.parse(
      // Auto generated from flowToTs. Please clean me!
      window.sessionStorage.correct_choice_depth !== null &&
        window.sessionStorage.correct_choice_depth !== undefined
        ? window.sessionStorage.correct_choice_depth
        : "{}"
    );
    if (correct_choice === null || choice === correct_choice) {
      if (correct_choice !== null) {
        ...
        window.sessionStorage.correct_choice_depth = JSON.stringify(
          correct_choice_depth
        );
      }
  ...
  render() {
    ...
    const choices_list = Object.keys(this.props.question.choices).map(
      (choice_index) => {
        return (
          <ChoiceButton
    ...
    return (
      <section className="centered-text">
        ...
        <div className="pure-g">{choices_list}</div>
        {this.renderNotice()}
      </section>
    );
  }
