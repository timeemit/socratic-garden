...
export default class ChallengeView extends React.Component<Props, State> {
  ...
  nextQuestion() {
    return QuestionByLessonID(
      this.props.lesson.id,
      this.state.finished.answered.concat(this.state.finished.skipped)
    );
  }
  ...
  onContinue = () => {
    ...
    return Router.push(
      "/lessons/[id]/[slug]",
      `/lessons/${lesson.id}/${slug(lesson.title)}`
    );
  };
  ...
  renderQuestion() {
    ...
    if (question == null) {
      return (
        <strong>No one has created a question for this lesson yet.</strong>
      );
    }
    return (
      <QuestionView
  ...
  renderNavigation() {
    ...
    let navigation_question = {
      ...
      lesson_id:
        ...
        (question === null || question === undefined
          ? undefined
          : question.lesson_id) !== null && // Auto generated from flowToTs. Please clean me!
        (question === null || question === undefined
          ? undefined
          : question.lesson_id) !== undefined // Auto generated from flowToTs. Please clean me!
          ? question === null || question === undefined
            ? undefined
            : question.lesson_id
          : -1,
    ...
    if (question == null || next_question == null) {
      ...
      navigation_question.choices[ChoiceIndices.first].text = NextLesson(
        this.props.lesson.id
      ).title;
      return (
        <QuestionView
    ...
    if (finished.answered.includes(question.id)) {
      ...
      navigation_question.choices[ChoiceIndices.first].text =
        next_question.text;
      return (
        <QuestionView
          key={-2}
    ...
    return (
      <em>
        <button className={styles.buttonLink} onClick={this.onSkip}>
          Skip this question
        </button>
      </em>
    );
