...
export default class ConceptLink extends React.Component<Props> {
  render() {
    ...
    return (
      <Link
        href="/concepts/[id]/[slug]"
        as={`/concepts/${this.props.concept.id}/${slug(
          this.props.concept.text
        )}`}
      >
        <a className={`pure-button ${styles.conceptLink} ${disabled}`}>
          {this.props.concept.as}
        </a>
      </Link>
    );
  }
