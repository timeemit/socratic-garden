...
export default class ConceptPage extends React.Component<Params> {
  ...
  renderTextPreview(lesson: LessonType) {
    const first_paragraph_section = lesson.sections.find(
      (section) => section.type === LESSON_PARAGRAPH
    );
    if (
      first_paragraph_section == null ||
      first_paragraph_section.type !== LESSON_PARAGRAPH
    ) {
      return null;
    }
    const first_text_section = first_paragraph_section.content.find(
      (span) => span.type === LESSON_TEXT
    );
